...
with description(playbooks.StartSysdigCaptureForContainerS3) as self:
    with before.each:
        ...
        self.playbook = playbooks.StartSysdigCaptureForContainerS3(self.k8s_client,
    ...
    with it('add starts capturing job in same node than Pod alerted'):
        ...
        expect(self.k8s_client.start_sysdig_capture_for)\
            .to(have_been_called_with('s3', pod_name,
                                      event_time,
                                      self.duration_in_seconds,
                                      self.s3_bucket,
                                      self.aws_access_key_id,
